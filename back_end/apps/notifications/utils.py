...
def create_and_send_notification(recipient, sender, type, message=None, community=None, image_url=None, product=None,post=None,):
    try:
        ...
        logger.exception(f"Failed to save notification for recipient {recipient.id if recipient else 'Unknown'}: {e}")
        return 
    ...
    try:
        ...
        logger.exception(f"Failed to send real-time notification to user {recipient.id if recipient else 'Unknown'}: {e}")
